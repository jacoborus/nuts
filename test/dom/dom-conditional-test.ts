...
test('DOM: renderIfElseConst', t => {
  const render = renderIfElseConst(box => !!box.x, [renderTextPlain('hola'), renderTag('span', [], [])])
  const box = getBox({ x: 1 })
  const comp = render(box)
...
test('DOM: renderIfElseVar', t => {
  const render = renderIfElseVar(box => !!box.x, ['x'], [renderTextPlain('hola'), renderTextPlain('adios')])
  const box = getBox({ x: 1 })
  const results = [
    'adios',
    'hola'
