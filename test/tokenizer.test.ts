import { tokenizeExpression, tokenizeHtml } from "../src/tokenizer.ts";
import { Reader } from "../src/reader.ts";
import { IToken, TokenKind } from "../src/types.ts";
...
let tests: {
  ...
  name: "tokenize expression: func prefix",
  expression: true,
  input: "@uno.dos ",
  result: [
  ...
  name: "tokenize expression: ctx prefix",
  expression: true,
  input: "$uno.dos ",
  result: [
...
tests.forEach((test) => {
  ...
  Deno.test(test.name, () => {
    const reader = new Reader(test.input);
    if (test.expression) tokenizeExpression(reader);
    else tokenizeHtml(reader);
    if (test.only) {
      console.log(test.result);
      console.log("===");
      console.log(reader.getTokens());
    }
    assertEquals(reader.getTokens(), test.result);
  });
});
