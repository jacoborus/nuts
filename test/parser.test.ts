import { parse, parseExpression, Reader } from "../src/parser.ts";
import { ElemSchema, ExprScope, NodeType, TokenKind } from "../src/types.ts";
import { assertEquals } from "https://deno.land/std@0.220.1/assert/mod.ts";

Deno.test("parser: simple text node element", () => {
  // '  hola'
  const tokens = [
  ...
  assertEquals(result, schema as ElemSchema[]);
});

Deno.test("parser: simple text + void tag", () => {
  // '  <br>'
  const tokens = [
...
Deno.test({
  name: "parser: simple tag + simple text",
  fn: () => {
    // '<span>hola</span>'
    const tokens = [
    ...
    const result = parse(new Reader(tokens));
    assertEquals(result, schema as unknown as ElemSchema[]);
  },
});

Deno.test("parser: void tag with regular attributes + void tag", () => {
...
Deno.test("parser: void tag with dynamic attributes", () => {
  // '<input :id="myid">'
  const tokens = [
    { start: 0, end: 0, type: TokenKind.OpenTag, value: "<" },
    { start: 1, end: 5, type: TokenKind.TagName, value: "input" },
...
Deno.test("parse expression: simple expression", () => {
  // '   uno.dos'
  const tokens = [
...
Deno.test("tokenize expression: func prefix", () => {
  //'@uno.dos '
  const tokens = [
...
Deno.test("tokenize expression: ctx prefix", () => {
  // '$uno.dos '
  const tokens = [
...
Deno.test("tokenize expression: single subexpression", () => {
  // 'uno.2.[ $dos.tres] '
  const tokens = [
...
Deno.test({
  name: "tokenize expression: multiple subexpression",
  ignore: true,
  fn: () => {
    // '{uno.2.{ $dos.tres}.{foo}} '
    const tokens = [
...
Deno.test({
  name: "tokenize expression: call function",
  ignore: true,
  fn: () => {
    // '@uno(dos.a,  dos.b) }'
    const tokens = [
