...
test('Build#Conditional if-else-const', t => {
  ...
  const result = "renderIfElseConst(box => !!box.x?.y,[renderTag('span',[],[]),renderTag('div',[],[])])"
  const str = buildConditional(tagSchema as CondSchema)
  t.is(str, result)
  t.end()
})
...
test('Build#Conditional if-var', t => {
  ...
  const result = "renderIfVar(box => !!box.x?.y,['x.y'],renderTag('span',[],[]))"
  const str = buildConditional(tagSchema as CondSchema)
  t.is(str, result)
  t.end()
})
...
test('Build#Conditional if-else-var', t => {
  ...
  const result = "renderIfElseVar(box => !!box.x?.y,['x.y'],[renderTag('span',[],[]),renderTag('div',[],[])])"
  const str = buildConditional(tagSchema as CondSchema)
  t.is(str, result)
  t.end()
})
