import { ElemSchema, CondSchema, AttSchema, TagSchema } from "../common";
...
function tagHasConditional(schema: ElemSchema): boolean {
  ...
  return (schema as TagSchema).attribs.some(
    ({ kind }) => kind === "conditionalConst" || kind === "conditionalVar"
  );
}
...
export function transformConditional(schema: TagSchema): CondSchema {
  ...
  const conditions = attrib.propName === "(if)" ? [getCondition(value)] : [];
...
export function cleanConditionalAttributes(schema: TagSchema): AttSchema[] {
  return schema.attribs.filter(
    (att) => att.propName !== "(if)" && att.propName !== "(else)"
  );
}
